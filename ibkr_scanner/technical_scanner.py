...
ENHANCED PRICE FETCHING (v2.3):
- Improved market data subscription with longer timeouts (3 seconds)
- Multiple fallback methods: marketPrice, last, close, historical data
- Better error handling and retry logic for price fetching
- Progress tracking and price summary reporting
"""
...
def save_scanner_results(long_stocks, short_stocks, args, ib_connection=None):
    """Save scanner results to JSON file with enhanced price fetching"""
    try:
        results = {
        ...
        def get_current_price(symbol, ib_conn):
            ...
            try:
                ...
                try:
                    # Request contract details to populate conId
                    contract_details = ib_conn.reqContractDetails(contract)
                    if contract_details and len(contract_details) > 0:
                        ...
                        print(f"      [SUCCESS] Contract qualified: {symbol} (conId: {qualified_contract.conId})")
                    else:
                        print(f"      [WARNING] No contract details found for {symbol}")
                        return 0
                except Exception as qual_e:
                    print(f"      [WARNING] Contract qualification failed for {symbol}: {qual_e}")
                    return 0
                
                # Request market data with longer timeout
                ticker = ib_conn.reqMktData(qualified_contract)
                
                # Wait longer for data to arrive (up to 3 seconds)
                max_wait = 3.0
                wait_time = 0.0
                ...
                # If no price after waiting, try to get last price
                if ticker.last and ticker.last > 0:
                    price = ticker.last
                    ib_conn.cancelMktData(qualified_contract)
                    return price
                
                # If still no price, try to get close price
                ...
                # Try alternative: get historical data for recent price
                try:
                    ...
                    print(f"      [WARNING] {symbol}: Historical data fallback failed: {hist_e}")
                
                return 0
                
            except Exception as e:
                print(f"      [ERROR] {symbol}: Market data error: {e}")
                return 0
        
        # Add long opportunities with current prices
        print("[PRICES] Fetching current prices for long opportunities...")
        for i, stock in enumerate(long_stocks):
            try:
                print(f"   [SCAN] {stock['symbol']} ({i+1}/{len(long_stocks)})...")
                current_price = get_current_price(stock['symbol'], ib_connection)
                
                if current_price > 0:
                    print(f"   ✅ {stock['symbol']}: ${current_price:.2f}")
                else:
                    print(f"   ⚠️  {stock['symbol']}: No price data available")
                ...
                print(f"   ❌ {stock['symbol']}: Error getting price: {e}")
                results['long'].append({
                    'symbol': stock['symbol'],
                    'score': stock['score'],
                    'rank': stock['rank'],
        ...
        # Add short opportunities with current prices
        print("\n[PRICES] Fetching current prices for short opportunities...")
        for i, stock in enumerate(short_stocks):
            try:
                print(f"   [SCAN] {stock['symbol']} ({i+1}/{len(short_stocks)})...")
                current_price = get_current_price(stock['symbol'], ib_connection)
        ...
        # Create scanner_results folder if it doesn't exist
        results_folder = "scanner_results"
        if not os.path.exists(results_folder):
            os.makedirs(results_folder)
            print(f"[FOLDER] Created folder: {results_folder}")
        
        # Save to file in the scanner_results folder
        filename = f"scanner_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        filepath = os.path.join(results_folder, filename)
        
        with open(filepath, 'w') as f:
            json.dump(results, f, indent=2)
        
        print(f"\n[SAVE] Scanner results saved to: {filepath}")
        print(f"   Use: python auto_day_trader.py --scanner-file {filepath}")
        
        # Print price summary
        long_with_prices = [s for s in results['long'] if s['current_price'] > 0]
        short_with_prices = [s for s in results['short'] if s['current_price'] > 0]
        
        print(f"\n[SUMMARY] Price Data Summary:")
        print(f"   Long opportunities with prices: {len(long_with_prices)}/{len(results['long'])}")
        print(f"   Short opportunities with prices: {len(short_with_prices)}/{len(results['short'])}")
        
        if long_with_prices:
            avg_long_price = sum(s['current_price'] for s in long_with_prices) / len(long_with_prices)
            print(f"   Average long price: ${avg_long_price:.2f}")
        
        if short_with_prices:
            avg_short_price = sum(s['current_price'] for s in short_with_prices) / len(short_with_prices)
            print(f"   Average short price: ${avg_short_price:.2f}")
        
    except Exception as e:
        print(f"[ERROR] Could not save scanner results: {e}")

def main():
    parser = argparse.ArgumentParser(description="Customizable Technical Analysis Stock Scanner for Long & Short Opportunities",
                                   formatter_class=argparse.RawDescriptionHelpFormatter,
                                   epilog="""
CONNECTION EXAMPLES:
  Paper Trading:        python technical_scanner.py --paper
  Live Trading:         python technical_scanner.py --live
  Default (Live):       python technical_scanner.py

INDICATOR CUSTOMIZATION EXAMPLES:
  Default settings:     python technical_scanner.py
  Fast MACD:           python technical_scanner.py --macd-fast 8 --macd-slow 21 --macd-signal 5
  Sensitive RSI:       python technical_scanner.py --rsi-period 9 --rsi-oversold 25 --rsi-overbought 75
  Strict ADX:          python technical_scanner.py --adx-period 10 --adx-threshold 30
  Conservative CCI:    python technical_scanner.py --cci-period 30 --cci-oversold -150 --cci-overbought 150
...
OUTPUT:
  ...
  1. LONG OPPORTUNITIES: Stocks with bullish momentum signals
  2. SHORT OPPORTUNITIES: Stocks with bearish reversal signals
""")
    ...
    parser.add_argument("--paper", action="store_true", help="Connect to paper trading (port 7497)")
    ...
    # Determine port based on paper/live flags
    if args.paper and args.live:
        print("Error: Cannot specify both --paper and --live. Please choose one.")
        return
    
    if args.paper:
        port = 7497
        print("Connecting to PAPER TRADING (port 7497)")
    elif args.live:
        port = 7496
        print("Connecting to LIVE TRADING (port 7496)")
    else:
        port = 7496  # Default to live trading
        print("No trading mode specified, defaulting to LIVE TRADING (port 7496)")
    ...
    # Initialize IB connection with error handling and retry logic
    ib = IB()
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            print(f"[CONNECT] Connecting to IB (attempt {attempt + 1}/{max_retries})...")
            ib.connect(args.host, port, clientId=args.client_id)
            
            # Wait for connection to stabilize
            print("[WAIT] Waiting for connection to stabilize...")
            time.sleep(3)
            
            # Test connection with a simple request
            print("[TEST] Testing IB connection...")
            ib.reqCurrentTime()
            time.sleep(1)
            
            print("[SUCCESS] IB connection established successfully!")
            break
            
        except Exception as e:
            error_msg = str(e)
            print(f"[ERROR] Connection attempt {attempt + 1} failed: {error_msg}")
            
            if "Error 162" in error_msg or "Historical Market Data Service error" in error_msg:
                print("[WARNING] Detected Error 162 - IB connection issue detected")
                if attempt < max_retries - 1:
                    print(f"[RETRY] Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)
                    retry_delay *= 2  # Exponential backoff
                else:
                    print("[ERROR] Max retries reached - attempting to continue with limited functionality")
            else:
                ...
                if attempt < max_retries - 1:
                    ...
                    print("[ERROR] Max retries reached - cannot continue")
                    return
    
    print("="*90)
    print("US STOCK TECHNICAL MOMENTUM SCANNER - LONG & SHORT OPPORTUNITIES")
    print("="*90)
    ...
    # Get scan results for US stocks with error handling
    max_scan_retries = 3
    scan_retry_delay = 3  # seconds
    
    for scan_attempt in range(max_scan_retries):
        try:
            print(f"[SCAN] Requesting scanner data (attempt {scan_attempt + 1}/{max_scan_retries})...")
            items = ib.reqScannerData(sub)
            
            if items:
                print(f"[SUCCESS] Scanner data received: {len(items)} stocks found")
                break
            else:
                print("[WARNING] No scanner data received")
                if scan_attempt < max_scan_retries - 1:
                    print(f"[RETRY] Retrying scanner request in {scan_retry_delay} seconds...")
                    time.sleep(scan_retry_delay)
                else:
                    print("[ERROR] Max scanner retries reached - using empty results")
                    items = []
                    break
                    
        except Exception as e:
            error_msg = str(e)
            print(f"[ERROR] Scanner request attempt {scan_attempt + 1} failed: {error_msg}")
            
            if "Error 162" in error_msg or "Historical Market Data Service error" in error_msg:
                print("[WARNING] Detected Error 162 - Scanner subscription cancelled")
                if scan_attempt < max_scan_retries - 1:
                    print(f"[RETRY] Retrying scanner request in {scan_retry_delay} seconds...")
                    time.sleep(scan_retry_delay)
                    scan_retry_delay *= 1.5  # Gradual backoff
                else:
                    print("[ERROR] Max scanner retries reached - attempting to continue with limited functionality")
                    items = []
                    break
            else:
                print(f"[WARNING] Unexpected scanner error: {error_msg}")
                if scan_attempt < max_scan_retries - 1:
                    print(f"[RETRY] Retrying scanner request in {scan_retry_delay} seconds...")
                    time.sleep(scan_retry_delay)
                else:
                    print("[ERROR] Max scanner retries reached - cannot continue")
                    return
    
    print(f"\nAnalyzing top {min(len(items), args.max_results)} US stocks...")
    if args.verbose:
    ...
    # Analyze each US stock for both long and short opportunities
    for i, item in enumerate(items[:args.max_results]):
        try:
            # Create contract
            contract = Stock(item.contractDetails.contract.symbol, 'SMART', 'USD')
            
            # Get historical data with custom interval and error handling
            max_data_retries = 2
            data_retry_delay = 2  # seconds
            ...
            for data_attempt in range(max_data_retries):
                try:
                    ...
                    error_msg = str(e)
                    if "Error 162" in error_msg or "Historical Market Data Service error" in error_msg:
                        print(f"[WARNING] {item.contractDetails.contract.symbol}: Error 162 on historical data (attempt {data_attempt + 1})")
                        if data_attempt < max_data_retries - 1:
                            time.sleep(data_retry_delay)
                        else:
                            print(f"   [ERROR] Skipping {item.contractDetails.contract.symbol} due to persistent Error 162")
                    else:
                        print(f"[WARNING] {item.contractDetails.contract.symbol}: Historical data error: {error_msg[:50]}")
                        if data_attempt < max_data_retries - 1:
                            time.sleep(data_retry_delay)
                        else:
                            print(f"   [ERROR] Skipping {item.contractDetails.contract.symbol} due to data error")
                    break
    ...
    # Summary
    print("\n" + "="*90)
    print(f"QUALIFIED LONG OPPORTUNITIES (Score >= {args.min_score}) - EXCLUDING CONFLICTING SIGNALS")
    print("="*90)
    ...
    if qualified_stocks:
        ...
        print("No stocks met the minimum long momentum score criteria.")
    ...
    if short_opportunities:
        ...
        print("No stocks met the minimum short opportunity score criteria.")
    ...
    if conflicting_symbols:
        ...
        print("="*90)
        print("These stocks showed both long and short signals and were excluded for clarity.")
        ...
        print("="*90)
        print("All qualified stocks showed clear directional signals.")

    # Save results for auto-trader
    save_scanner_results(qualified_stocks, short_opportunities, args, ib)

    # Graceful disconnect with error handling
    try:
        print("[DISCONNECT] Disconnecting from IB...")
        ib.disconnect()
        print("[SUCCESS] Disconnected successfully")
    except Exception as e:
        error_msg = str(e)
        if "Error 162" in error_msg or "Historical Market Data Service error" in error_msg:
            print("[WARNING] Error 162 during disconnect - this is normal and expected")
        else:
            print(f"[WARNING] Disconnect error: {error_msg}")
        print("[SUCCESS] Scanner completed despite disconnect error")
